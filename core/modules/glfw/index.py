...
def __fix_vcproj():
    if not platform.system() == "Windows":
        return
    file = Builder(module_dir+'/'+m_config.vis_studio_project_path)
    config = file.get_configuration("Debug")
    config.set_runtime_library(Builder.runtimeLibraries["MDd"])
    config.save()
    config = config.get_configuration("Release")
    config.set_runtime_library(Builder.runtimeLibraries["MD"])
    config.save()
...
def compile_glew(sources_dir, params):
    if platform.system() == "Windows":
        ...
        project = Builder(module_dir+'/'+m_config.vis_studio_project_path)
